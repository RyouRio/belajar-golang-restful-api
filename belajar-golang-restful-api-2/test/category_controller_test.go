...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root:fredrick3246@tcp(localhost:3306)/belajar_golang_restful_api_test")
...
func TestUpdateCategorySuccess(t *testing.T) {
	...
	assert.Equal(t, "Gadget Baru", responseBody["data"].(map[string]any)["name"])
}
...
func TestGetCategoryFailed(t *testing.T) {
	...
	router := setupRouter(db)
	request := httptest.NewRequest("GET", "http://localhost:3000/api/categories/404", nil	)
	request.Header.Add("X-API-KEY", "RAHASIA")
...
func TestDeleteCategoryFailed(t *testing.T){
	...
	router := setupRouter(db)
	request := httptest.NewRequest("DELETE", "http://localhost:3000/api/categories/404", nil)
	request.Header.Add("Content-Type", "application/json")
	request.Header.Add("X-API-KEY", "RAHASIA")
